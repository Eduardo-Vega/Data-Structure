...
void main()
{
    ...
    do
    {
        ...
        case 1:
            ...
            printf("\nIngrese monto de la venta en la ISLA %d del COMBUSIBLE %s en LITROS: ", isla, combustible[tipoCombustible-1]); //SE LEE EL MONTO EN LITROS DEL DESPACHADOR SELECCIONADO
            scanf("%f", &monto);
            //SE RESTA EL MONTO INGRESADO AL MONTO ACTUAL DEL DESPACHADOR ( 5000(o restante) - MONTO)
            gas.diaSemana[dia].islaN[isla-1].despachadorN.tipoCombustible[tipoCombustible-1] = gas.diaSemana[dia].islaN[isla-1].despachadorN.tipoCombustible[tipoCombustible-1] - monto;
            break;
        case 2:
